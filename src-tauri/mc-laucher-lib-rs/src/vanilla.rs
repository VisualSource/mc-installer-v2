use crate::utils::{ parse_rule_list, download_file, read_manifest_inherit, get_offical_version_list };
...
fn install_libraries(id: String, libraries: &Vec<Library>, path: PathBuf, callback: Callback ) -> LibResult<()> {
    ...
    for (count, i) in libraries.iter().enumerate() {
        ...
        if let Err(err) = download_file(download_url, current_path.join(jar_filename.clone()), callback, None,false) {
            error!("{}",err);
        }
        ...
        if let Some(downloads) = &i.downloads {
            ...
            if let Err(err) = download_file(downloads.artifact.url.clone(), current_path.join(jar_filename), callback,Some(downloads.artifact.sha1.clone()), false) {
                return Err(err);
            }
            ...
            if !native.is_empty() {
                if let Some(classifiers) = &downloads.classifiers {
                    if let Some(nat) = classifiers.get(&native) {
                        if let Err(err) = download_file(nat.url.clone(), current_path.join(jar_filename_native.clone()), callback, Some(nat.sha1.clone()), false) {
                            return Err(err);
                        }
                    }
                    if let Some(extract) = &i.extract {
...
fn install_assets(manifest: &VersionManifest, path: PathBuf, callback: Callback) -> LibResult<()> {
    ...
    if let Some(asset_index) = &manifest.asset_index {
        if let Err(err) = download_file(asset_index.url.clone(), index_path.clone(), callback, Some(asset_index.sha1.clone()), false) {
            return Err(err);
        }

        let assets: IndexAssetsMap = match read_to_string(index_path) {
            Ok(raw) => {
        ...
        for (key, value) in assets.objects {
            ...
            if let Err(err) = download_file(url, outpath, callback, Some(value.hash.clone()), false){
                return Err(err);
            }
            count += 1;
            callback(Event::progress(count,max));
...
fn do_version_install(version_id: String, path: PathBuf, callback: Callback, url: Option<String>) -> LibResult<()> {
    ...
    if let Some(url_d) = url {
        if let Err(err) = download_file(url_d, version_manifest.clone(), callback, None, false) {
            return Err(err);
        }
    }

    
    let manifest: VersionManifest = match read_manifest_inherit(version_manifest,&path) {
        Ok(value) => value,
        Err(err) => return Err(err)
    };

    callback(Event::Status("Installing libraries".into()));
    if let Err(err) = install_libraries(manifest.id.clone(), &manifest.libraries, path.clone(), callback) {
        return Err(err);
    }

    callback(Event::Status("Installing Assets".into()));
    if let Err(err) = install_assets(&manifest, path.clone(), callback) {
        return Err(err);
    }   
    ...
    if let Some(logging) = manifest.logging {
        ...
        if let Some(client) = logging.get("client") {
            if let Some(id) = &client.file.id {
                ...
                if let Err(err) = download_file(client.file.url.clone(), logging_file, callback, Some(client.file.sha1.clone()), false) {
                    return Err(err);
                }
            }
        }
    ...
    if let Some(downloads) = manifest.downloads {
        ...
        if let Some(client) = downloads.get("client") {
            if let Err(err) = download_file(client.url.clone(), path.join("versions").join(manifest.id.clone()).join(format!("{}.jar",manifest.id.clone())), callback, Some(client.sha1.clone()), false) {
                return Err(err);
            }
        }
    }
    ...
    if let Some(java) = manifest.java_version {
        ...
        match does_runtime_exist(java.component.clone(), path.clone()) {
            Ok(value) => {
                if !value {
                    if let Err(err) = install_jvm_runtime(java.component, path, callback) {
                        return Err(err);
                    }
                }
            }
...
pub fn install_minecraft_version(version_id: String, mc_dir: PathBuf, callback: Callback) -> LibResult<()> {
    if mc_dir.join("versions").join(version_id.clone()).join(format!("{}.json",version_id)).is_file() {
       return do_version_install(version_id, mc_dir, callback, None);
    }
    match get_offical_version_list() {
        Ok(versions) => {
            ...
            if let Some(item) = version.get(0) {
                return do_version_install(item.id.clone(), mc_dir, callback, Some(item.url.clone()));
            }
